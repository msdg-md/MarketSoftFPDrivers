        ...
        protected byte[] SendCommand(Command command, byte[] extendedData)
		{
			...
			while(true)
			{
				try
				{
					...
					Log.Write(String.Format("Отправка команды на фискальный принтер: {0} пакет({1})", command, BitConverter.ToString(packet)) ,Log.MessageType.Info, this);
...
*/
		...
		{
			Log.Write("Получение ответа от фискального принтера." ,Log.MessageType.Info, this);
		...
		private byte[] ReceivePacket()
		{
			...
			while(true)
			{
				...
				{
					Log.Write(String.Format("Получен пакет: ({0})", BitConverter.ToString((byte[])packet.ToArray(typeof(byte)))) ,Log.MessageType.Info, this);

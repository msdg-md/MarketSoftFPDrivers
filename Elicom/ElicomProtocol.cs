...
namespace Phoenix.Devices.Printers.Elicom
{
    public class ElicomProtocol
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine(((cmds)packet.Command).ToString("F"));
            if (!IsOpen)
        ...
        public virtual void PrintLine(string line)
        {
            ...
            if (line.Length > TextDocLineLength)
                line = line.Substring(0, TextDocLineLength);

            SendPacket(new Packet(Commands.PrintTextLine, line));
        }
        ...
        public void AddRefundReceiptItem(string atrName, Money unitPrice, Quantity count, TaxGrp taxGrp, Money disc)
        {
            Packet packet = new Packet(Commands.Storno);
            packet.AddMessage(NormalizeString(atrName, 36));
            packet.AddMessage(GetTaxGrp(taxGrp));
